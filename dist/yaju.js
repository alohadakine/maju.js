// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju = function() {};

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.after = function(el, afterEl) {
    el.parentNode.insertBefore(afterEl, el.nextElementSibling);
    return el;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.ajax = function(opts) {
    var formatParams, getTimestamp, header, params, xhr;
    opts = opts || {};
    opts.method = opts.method || 'GET';
    opts.method = opts.method.toUpperCase();
    opts.cb = opts.cb || function() {};
    opts.params = opts.params || {};
    opts.requestContentType = opts.requestContentType || 'application/x-www-form-urlencoded';
    getTimestamp = function() {
      return Math.floor(Date.now() / 1000);
    };
    formatParams = function(params) {
      var arr, key;
      arr = [];
      for (key in params) {
        arr.push(encodeURIComponent(key) + '=' + encodeURIComponent(params[key]));
      }
      return arr;
    };
    params = formatParams(opts.params);
    xhr = new XMLHttpRequest();
    if (opts.method === 'GET' && params.length) {
      opts.url = opts.url + '?' + params.join('&');
    }
    if (opts.method === 'GET' && opts.cache === false) {
      if (opts.url.indexOf('?') === -1) {
        opts.url = opts.url + '?_nocache=' + getTimestamp();
      } else {
        opts.url = opts.url + '&_nocache=' + getTimestamp();
      }
    }
    xhr.open(opts.method, opts.url);
    xhr.setRequestHeader('Content-type', opts.requestContentType);
    if (opts.headers) {
      for (header in opts.headers) {
        xhr.setRequestHeader(header.key, header.value);
      }
    }
    if (opts.method === 'GET') {
      xhr.send(null);
    } else {
      if (params.length) {
        xhr.send(params.join('&'));
      } else {
        xhr.send(null);
      }
    }
    xhr.onreadystatechange = function() {
      var DONE, OK, err;
      DONE = 4;
      OK = 200;
      err = false;
      if (xhr.readyState === DONE) {
        if (xhr.status !== OK) {
          err = xhr.status;
        }
        opts.cb(err, xhr.responseText);
      }
    };
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.append = function(el, appendEl) {
    el.appendChild(appendEl);
    return el;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.attr = function(el, key, value) {
    var returnvalue;
    returnvalue = el;
    if (value) {
      el.setAttribute(key, value);
    } else {
      returnvalue = el.getAttribute(key);
    }
    return returnvalue;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.before = function(el, beforeEl) {
    el.parentNode.insertBefore(beforeEl, el);
    return el;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.create = function(name) {
    var el;
    el = document.createElement(name);
    return window.yaju.find(el).get(0);
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.domready = function(cb) {
    if (document.readyState === 'complete') {
      return window.setTimeout(cb, 1);
    } else {
      document.addEventListener('DOMContentLoaded', function() {
        cb();
      });
    }
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.each = function(arr, cb) {
    var child, i, j, len;
    for (i = j = 0, len = arr.length; j < len; i = ++j) {
      child = arr[i];
      cb(child, i);
    }
    return arr;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.find = function(sel, ref) {
    var el, el_arr, els, i, len;
    el_arr = [];
    if (sel) {
      if (typeof sel !== 'object') {
        if (ref) {
          els = ref.querySelectorAll(sel);
        } else {
          els = document.querySelectorAll(sel);
        }
      } else {
        if (sel.length) {
          els = sel;
        } else {
          els = [sel];
        }
      }
      for (i = 0, len = els.length; i < len; i++) {
        el = els[i];
        el.find = function(sel) {
          return window.yaju.find(sel, this);
        };
        el.on = function(eventName, cb) {
          return window.yaju.on(this, eventName, cb);
        };
        el.attr = function(key, value) {
          return window.yaju.attr(this, key, value);
        };
        el.html = function(value) {
          return window.yaju.html(this, value);
        };
        el.prepend = function(prependEl) {
          return window.yaju.prepend(this, prependEl);
        };
        el.append = function(appendEl) {
          return window.yaju.append(this, appendEl);
        };
        el.after = function(afterEl) {
          return window.yaju.after(this, afterEl);
        };
        el.before = function(beforeEl) {
          return window.yaju.before(this, beforeEl);
        };
        el.parent = function() {
          return window.yaju.parent(this);
        };
        el.next = function() {
          return window.yaju.next(this);
        };
        el.prev = function() {
          return window.yaju.prev(this);
        };
        el.nodes = function() {
          return window.yaju.nodes(this);
        };
        el_arr.push(el);
      }
      el_arr.each = function(cb) {
        return window.yaju.each(this, cb);
      };
      el_arr.get = function(index) {
        return this[index];
      };
    }
    return el_arr;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.html = function(el, value) {
    var returnvalue;
    returnvalue = el;
    if (value) {
      el.innerHTML = value;
    } else {
      return el.innerHTML;
    }
    return returnvalue;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.loadjs = function(url, cb) {
    var s, y;
    y = window.yayu;
    s = y.create('script').attr('type', 'text/javascript').attr('async', 'true').attr('src', url);
    if (cb) {
      s.on('load', function() {
        cb();
      });
    }
    y.find('script').get(0).prepend(s);
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.loadjs = function(urls, cb) {
    var appendscript, i, len, url, y;
    y = window.yaju;
    appendscript = function(url, cb) {
      var s;
      s = y.create('script').attr('type', 'text/javascript').attr('async', 'true').attr('src', url);
      if (cb) {
        s.on('error', function() {
          cb(true, this);
        });
        s.on('load', function() {
          cb(false, this);
        });
      }
      return y.find('script').get(0).prepend(s);
    };
    if (typeof urls === 'string') {
      urls = [urls];
    }
    for (i = 0, len = urls.length; i < len; i++) {
      url = urls[i];
      appendscript(url, cb);
    }
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  var LocalStorage;

  LocalStorage = function() {
    var ApplyBackup, GetBackup, localStorage;
    localStorage = window.localStorage;
    ApplyBackup = function(backup, fClear, fOverwriteExisting) {
      var i, key, len, value;
      fClear = fClear || false;
      fOverwriteExisting = fOverwriteExisting || false;
      if (fClear === true) {
        localStorage.clear();
      }
      for (value = i = 0, len = backup.length; i < len; value = ++i) {
        key = backup[value];
        if (fOverwriteExisting === false && backup[key] !== void 0) {
          continue;
        }
        localStorage.setItem(key, value);
      }
    };
    GetBackup = function() {
      var backup, key, value;
      backup = {};
      for (key in localStorage) {
        value = localStorage[key];
        backup[key] = value;
      }
      return backup;
    };
    this.GetRemainingSpace = function(cb) {
      var data, e, increase, itemBackup, totalData, trytotalData;
      itemBackup = localStorage.getItem('');
      increase = true;
      data = '1';
      totalData = '';
      trytotalData = '';
      while (true) {
        try {
          trytotalData = totalData + data;
          localStorage.setItem('', trytotalData);
          totalData = trytotalData;
          if (increase) {
            data += data;
          }
        } catch (error) {
          e = error;
          if (data.length < 2) {
            break;
          }
          increase = false;
          data = data.substr(data.length / 2);
        }
      }
      if (itemBackup === null) {
        localStorage.removeItem('');
      } else {
        localStorage.setItem('', itemBackup);
      }
      cb(totalData.length);
    };
    this.GetMaximumSize = function(cb) {
      var backup, max;
      backup = GetBackup();
      localStorage.clear();
      max = this.GetRemainingSpace(function(max) {
        ApplyBackup(backup);
        cb(max);
      });
    };
    return this;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.next = function(el) {
    return window.yaju.find(el.nextElementSibling).get(0);
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.children = function(el) {
    var child, child_arr, i, len, ref;
    child_arr = [];
    ref = el.children;
    for (i = 0, len = ref.length; i < len; i++) {
      child = ref[i];
      child_arr.push(window.yaju.find(child).get(0));
    }
    return child_arr;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.on = function(el, eventName, cb) {
    el.addEventListener(eventName, cb, false);
    return el;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.parent = function(el) {
    return window.yaju.find(el.parentNode).get(0);
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.plugins = {};

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.prepend = function(el, prependEl) {
    el.parentNode.insertBefore(prependEl, el);
    return el;
  };

}).call(this);
// Generated by CoffeeScript 1.11.1
(function() {
  window.yaju.prototype.prev = function(el) {
    return window.yaju.find(el.previousElementSibling).get(0);
  };

}).call(this);
window.yaju = new window.yaju();
